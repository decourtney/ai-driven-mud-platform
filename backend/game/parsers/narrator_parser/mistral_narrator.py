...
class GGUFMistralNarrator(ActionNarrator):
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if not cuda_available and self.n_gpu_layers > 0:
                ...
                print("[!] To enable GPU: pip install llama-cpp-python --force-reinstall --no-cache-dir --config-settings cmake.define.LLAMA_CUBLAS=on")
                self.n_gpu_layers = 0
    ...
    def _create_input_prompt(self, action: ParsedAction, hit: bool, damage_type: str) -> str:
        ...
        if action.weapon and action.action_type == ActionType.ATTACK and action.weapon not in action_desc:
