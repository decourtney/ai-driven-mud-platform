...
class ModelServer:
    ...
    def _create_app(self) -> FastAPI:
        ...
        def detailed_status():
            ...
            return {
                ...
                "model_paths": {
                    ...
                    "narrator": getattr(self.model_manager, 'narrator_model_path', None),
                    "adapter": getattr(self.model_manager, 'narrator_adapter_path', None)
                }
            }
        ...
        def parse_action(request: ParseActionRequest):
            ...
            if not self.model_manager.is_parser_ready():
                ...
                if not self.model_manager.load_all_models():
                    raise HTTPException(status_code=503, detail="Parser model not available")
        ...
        @app.post("/generate_action", response_model=GenerateNarrationResponse)
        def generate_action_narration(request: GenerateActionRequest):
            if not self.model_manager.is_narrator_ready():
                ...
                if not self.model_manager.load_all_models():
                    raise HTTPException(status_code=503, detail="Narrator model not available")
            ...
            try:
                ...
                return GenerateNarrationResponse(narration=narration)
                
            except Exception as e:
                raise HTTPException(status_code=400, detail=f"Generate action failed: {e}")   
        
        @app.post("/generate_scene", response_model=GenerateNarrationResponse)  
        def generate_scene_narration(request: GenerateSceneRequest):
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Generate scene failed: {e}")   
        ...
        def batch_parse_actions(requests: List[ParseActionRequest]):
            ...
            for req in requests:
                try:
                    result = self.model_manager.parse_action(req.text, context=req.context)
                    results.append(ParseActionResponse(
                        success=True,
...
def main():
    ...
    parser.add_argument("--load-models", action="store_true", help="Load models at startup")
