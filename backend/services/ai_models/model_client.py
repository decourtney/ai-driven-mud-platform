...
from backend.models import (ParseActionResponse, ParseActionRequest, GenerateActionRequest, 
...
#             response = self.client.post(f"{self.base_url}/models/load")
#             response.raise_for_status()
...
#         try:
#             response = self.client.post(f"{self.base_url}/models/unload")
#             response.raise_for_status()
...
#         try:
#             response = self.client.post(f"{self.base_url}/models/reload")
#             response.raise_for_status()
...
#     def wait_for_service(self, timeout: float = 60.0, interval: float = 1.0) -> bool:
#         """Wait for model service to become available"""
#         print(f"[CLIENT] Waiting for model service at {self.base_url}...")
        
#         start_time = time.time()
#         while time.time() - start_time < timeout:
#             if self.is_healthy():
#                 print(f"[CLIENT] ✅ Model service is available!")
#                 return True
            
#             print(f"[CLIENT] Model service not ready, retrying in {interval}s...")
#             time.sleep(interval)
        
#         print(f"[CLIENT] ❌ Model service did not become available within {timeout}s")
#         return False
    
#     def ensure_models_loaded(self, auto_load: bool = True) -> bool:
...
class AsyncModelServiceClient:
    ...
    async def wait_for_service(self, timeout: float = 60.0, interval: float = 3.0) -> bool:
    ...
    async def parse_action(self, request: ParseActionRequest) -> ParseActionResponse:
        ...
        try:
            ...
            return ParseActionResponse(**result_dict)
    ...
    async def generate_action(self, request: GenerateActionRequest) -> GenerateNarrationResponse:
        ...
        try:          
            ...
            result_dict = response.json()
            result = GenerateNarrationResponse(**result_dict) 
            ...
            return GenerateNarrationResponse(action_type="unknown", details=error_detail)

        except Exception as e:
            print(f"[CLIENT] Generation request failed: {e}")
            return GenerateNarrationResponse(action_type="unknown", details=str(e))
            
    async def generate_scene(self, request: GenerateSceneRequest) -> GenerateNarrationResponse:
        """Async version of generate_narration"""
        try:          
            response = await self.client.post(
