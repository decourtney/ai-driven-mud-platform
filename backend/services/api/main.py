...
async def lifespan(app: FastAPI):
        ...
        if WAIT_FOR_MODELS:
            ...
            if await model_client.wait_for_service(timeout=60.0):
                ...
                if AUTO_LOAD_MODELS:
                    ...
                    print(
                        "[+] Models not auto-loaded (set AUTO_LOAD_MODELS=true to enable)"
                    )
            else:
                print(
                    "[-] ⚠️ Model service not available - API will work but models may fail"
                )
        else:
            print(
                "[+] Not waiting for model service (set WAIT_FOR_MODELS=true to enable)"
            )
    ...
    # START engine cleanup loop here
    await api_server.session_manager.start()
    ...
    # STOP engine cleanup loop here
    await api_server.session_manager.stop()
